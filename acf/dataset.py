...
class UniversalMultiModalDataset(data.Dataset):
        ...
        if self.dataset_name == 'vaihingen':
            ...
            for id in ids:
                ...
                for modality, pattern in config['file_patterns'].items():
                    # 参考项目的路径构建方式：data_path + 'Vaihingen' + pattern
                    # 如果data_path已经包含Vaihingen，则不重复添加
                    if 'Vaihingen' in self.data_path:
                        file_path = os.path.join(self.data_path, pattern.format(id))
                    else:
    ...
    def _get_vaihingen_item(self, i):
        ...
        expected_size = self.window_size  # 使用配置的窗口大小
        if rgb_patch.shape[1:] != expected_size:
            logger.warning(f"数据增强后尺寸不正确: {rgb_patch.shape[1:]}, 期望: {expected_size}")
            # 使用OpenCV进行resize
            import cv2
            if rgb_patch.shape[0] == 3:
                rgb_patch = np.transpose(rgb_patch, (1, 2, 0))
            ...
            # 检查尺寸是否有效
            if rgb_patch.shape[0] > 0 and rgb_patch.shape[1] > 0:
                rgb_patch = cv2.resize(rgb_patch, expected_size, interpolation=cv2.INTER_LINEAR)
            else:
                rgb_patch = np.zeros((expected_size[0], expected_size[1], 3), dtype=np.uint8)
            if len(rgb_patch.shape) == 3:
                rgb_patch = np.transpose(rgb_patch, (2, 0, 1))
            # 转换回float32并归一化
            rgb_patch = rgb_patch.astype(np.float32) / 255.0
            ...
            # 检查尺寸是否有效
            if dsm_patch.shape[0] > 0 and dsm_patch.shape[1] > 0:
                dsm_patch = cv2.resize(dsm_patch, expected_size, interpolation=cv2.INTER_LINEAR)
            else:
                dsm_patch = np.zeros(expected_size, dtype=np.uint8)
            if len(dsm_patch.shape) == 2:
                dsm_patch = dsm_patch[np.newaxis, :, :]
            # 转换回float32并归一化
            dsm_patch = dsm_patch.astype(np.float32) / 255.0
            ...
            # 检查标签尺寸是否有效
            if label_patch.shape[0] > 0 and label_patch.shape[1] > 0:
                label_patch = cv2.resize(label_patch.astype(np.uint8), expected_size, interpolation=cv2.INTER_NEAREST)
            else:
                label_patch = np.zeros(expected_size, dtype=np.uint8)
        ...
        # 最终尺寸验证 - 使用配置的窗口大小
        assert rgb_patch.shape == (3, expected_size[0], expected_size[1]), f"RGB patch shape错误: {rgb_patch.shape}"
        assert dsm_patch.shape == (1, expected_size[0], expected_size[1]), f"DSM patch shape错误: {dsm_patch.shape}"
        assert label_patch.shape == expected_size, f"Label patch shape错误: {label_patch.shape}"
    ...
    def convert_from_color(arr_3d, palette=None):
        ...
        if palette is None:
            ...
            palette = {
                ...
                # 注意：黑色(0,0,0)不在palette中，会被默认为类别0
            }
